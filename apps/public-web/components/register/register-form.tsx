...
export function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // §7.4 — public, unauthenticated, CORS-allowed mutation directly against the API Worker.
      // public-web has no runtime dependency on the API for page views; only this form calls it.
      ...
      if (res.ok && data.success) {
        ...
        setSubmitState({
          status: "error",
          message: data.message ?? "Too many attempts. Please try again in a little while.",
        });
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="mx-auto max-w-xl">
      ...
      <div className="space-y-5">
        {/* §30.4 honeypot — hidden from humans; bots that fill it get a 422 from the API */}
        <div
